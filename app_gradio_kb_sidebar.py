...
class RoutedKnowledgeBaseApp:
    ...
    def _extract_doc_profile(self, source_file: str, source_path: str, text: str) -> Dict[str, Any]:
        ...
        return {k: (v if v not in {"", None} else "unknown") for k, v in profile.items()}
    ...
    def _build_section_documents(
        ...
        for idx, section in enumerate(extract_markdown_sections(markdown_text), start=1):
            ...
            metadata.update({
                ...
                "section_id": f"{base_metadata.get('doc_id')}__sec_{idx:03d}" + (f"_p{page_number:04d}" if page_number else ""),
                "citation_label": (
    ...
    def _load_documents_from_manifest(self) -> List[Document]:
        ...
        for source_path, info in sorted(self.manifest.items(), key=lambda x: x[1]["source_file"].lower()):
            ...
            for warning in info.get("diagnostics", []) or []:
            ...
            if file_type == "pdf":
                ...
                metadata.update({
                    ...
                    "citation_label": info.get("base_metadata", {}).get("citation_label", source_file),
                    "file_type": "markdown",
                })
                docs.append(
                    Document(
                ...
                summary_docs.extend(self._build_section_documents(text, metadata, page_number=None))
    ...
    def ingest_all_documents(self, progress: Optional[gr.Progress] = None):
        ...
        for idx, file_path in enumerate(all_files, start=1):
            ...
            if ext == ".pdf":
                ...
                for page_number, page_pdf_path in page_pdfs:
                    ...
                    if normalized_hash and normalized_hash in seen_page_hashes:
                        file_diagnostics.append(
                            f"page {page_number} duplicates content from {seen_page_hashes[normalized_hash]}"
                        )
                    ...
                    if text:
                        ...
                        metadata.update({
                            "page_number": page_number,
                            "page_markdown_path": page_md_path,
                            "section_title": sections[0]["section_title"] if sections else "Document Overview",
                            "section_path": sections[0]["section_path"] if sections else "Document Overview",
                    ...
                    page_records.append({
                        ...
                        "page_markdown_path": page_md_path,
                        "doc_id": doc_id,
                        "metadata": metadata if text else dict(base_metadata, page_number=page_number, citation_label=f"{source_file} p.{page_number}"),
                    })
                ...
                doc_id = safe_stem(source_path)
                profile = self._extract_doc_profile(source_file, source_path, text)
                base_metadata = self._build_base_metadata(
                    source_file=source_file,
                    source_path=source_path,
                    file_type="markdown",
                ...
                if source_hash in seen_doc_hashes:
                    ...
                    seen_doc_hashes[source_hash] = source_file
                if len(normalize_text_for_display(text)) < MIN_MEANINGFUL_TEXT_LEN:
                    file_diagnostics.append("document is empty or nearly empty")
    ...
    def get_document_table(self) -> List[List[Any]]:
        ...
        for source_path, info in sorted(self.manifest.items(), key=lambda x: x[1]["source_file"].lower()):
            file_type = info.get("file_type")
            source_file = info.get("source_file")
            base_meta = info.get("base_metadata", {})
            warnings = " | ".join(info.get("diagnostics", [])[:3]) if info.get("diagnostics") else ""
            if file_type == "pdf":
                ...
                rows.append([
                    ...
                    "PDF",
                    page_count,
                    safe_meta_value(base_meta.get("document_type")),
                    safe_meta_value(base_meta.get("model_name")),
                    safe_meta_value(base_meta.get("policy_name")),
                    safe_meta_value(base_meta.get("version")),
                    warnings,
                    source_path,
                ])
            else:
    ...
    def get_metadata_filter_choices(self) -> Dict[str, List[str]]:
        def collect(field: str) -> List[str]:
            values = {
                safe_meta_value(info.get("base_metadata", {}).get(field))
                for info in self.manifest.values()
            }
            return sorted(v for v in values if v and v != "unknown")

        return {
            "source_files": self.get_document_names(),
            "document_types": collect("document_type"),
            "model_names": collect("model_name"),
            "policy_names": collect("policy_name"),
        }

    def get_ingestion_diagnostics_text(self) -> str:
        if not self.ingestion_diagnostics:
            return "No ingestion warnings."
        return "\n".join(self.ingestion_diagnostics[:50])
    ...
    def _score_confidence(
        ...
        source_file_count = len({c.get("source_file") for c in citations if c.get("source_file")})
        scores = [
            float(c["score"])
            for c in citations
        ...
        return {
            ...
            "cross_file_evidence": source_file_count > 1,
    ...
    def _run_mode_query(
        ...
        engine = self._build_engine_for_mode(
            mode,
            source_files,
            doc_ids,
            page_numbers,
    ...
    def _compose_answer_text(
        ...
        header_lines = [
            ...
            f"Evidence: {confidence.get('citation_count', 0)} citation(s), {confidence.get('page_grounded_count', 0)} page-grounded, {confidence.get('source_file_count', 0)} file(s)",
    ...
    def _format_answer_html(
        ...
        <div class="answer-block">
          ...
          <div class="answer-meta"><b>Evidence:</b> {html_escape(str(confidence.get('citation_count', 0)))} citation(s), {html_escape(str(confidence.get('page_grounded_count', 0)))} page-grounded, {html_escape(str(confidence.get('source_file_count', 0)))} file(s)</div>
        ...
        for c in citations:
            ...
            <details class="source-card" id="source-card-{c['rank']}">
              ...
              <div class="source-body">
                <div class="source-meta"><b>File:</b> {html_escape(str(c.get('source_file')))}</div>
                <div class="source-meta"><b>Page:</b> {html_escape(str(c.get('page_number')))}</div>
                <div class="source-meta"><b>Document type:</b> {html_escape(str(c.get('document_type')))}</div>
                <div class="source-meta"><b>Section:</b> {html_escape(str(c.get('section_title')))}</div>
                <div class="source-meta"><b>Policy:</b> {html_escape(str(c.get('policy_name')))}</div>
                <div class="source-meta"><b>Model:</b> {html_escape(str(c.get('model_name')))}</div>
                <div class="source-meta"><b>Version:</b> {html_escape(str(c.get('version')))}</div>
...
def ui_filter_updates():
    choices = KB.get_metadata_filter_choices()
    return (
        gr.update(choices=choices["source_files"], value=[]),
        gr.update(choices=choices["document_types"], value=None),
        gr.update(choices=choices["model_names"], value=None),
        gr.update(choices=choices["policy_names"], value=None),
    )
...
def ui_add_documents(files, progress=gr.Progress()):
    ...
    return (
        KB.get_document_table(),
        gr.update(choices=choices, value=None),
        "\n".join(parts + [KB.get_ingestion_diagnostics_text()]),
        *ui_filter_updates(),
    )
...
def ui_send_message(
    ...
    document_types = [selected_document_types] if selected_document_types else None
    model_names = [selected_model_names] if selected_model_names else None
    policy_names = [selected_policy_names] if selected_policy_names else None
    result = KB.answer(
        user_msg,
        mode=mode,
        source_files=source_files,
...
def build_ui():
    with gr.Blocks(title="Agentic RAG Chatbot with Cortex AI") as demo:
        ...
        with gr.Tabs():
            with gr.Tab("Documents"):
                ...
                doc_table = gr.Dataframe(
                    headers=["Document", "Type", "Pages", "Document Type", "Model Name", "Policy Name", "Version", "Warnings", "Path"],
                    ...
                    with gr.Column(scale=7, min_width=500):
                        ...
                        with gr.Row():
                            ...
                            source_filter = gr.Dropdown(
                                ...
                                choices=KB.get_metadata_filter_choices()["source_files"],
                                multiselect=True,
                                interactive=True,
                            )
                        with gr.Row():
                            document_type_filter = gr.Dropdown(
                                ...
                                choices=KB.get_metadata_filter_choices()["document_types"],
                                interactive=True,
                            )
                            model_name_filter = gr.Dropdown(
                                label="Filter by model name",
                                choices=KB.get_metadata_filter_choices()["model_names"],
                                interactive=True,
                            )
                            policy_name_filter = gr.Dropdown(
                                label="Filter by policy name",
                                choices=KB.get_metadata_filter_choices()["policy_names"],
                                interactive=True,
                            )
        ...
        add_btn.click(
            fn=ui_add_documents,
            inputs=[file_input],
            outputs=[doc_table, doc_delete_dropdown, doc_status, source_filter, document_type_filter, model_name_filter, policy_name_filter],
            show_progress="full",
        )
        ...
        clear_btn.click(
            ...
            inputs=[],
            outputs=[doc_table, doc_delete_dropdown, doc_status, source_filter, document_type_filter, model_name_filter, policy_name_filter, thread_dropdown, thread_sidebar, chatbot, thread_status, sources_panel],
        )
        ...
        send_btn.click(
            fn=ui_send_message,
            inputs=[thread_dropdown, message_box, chatbot, mode_selector, source_filter, document_type_filter, model_name_filter, policy_name_filter],
